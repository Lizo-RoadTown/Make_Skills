 ...
 *
 * KPIs, charts, and recent-activity panels. The previous /observability
 * page body, minus the outer header (now in the shell).
 */
...
export function OverviewTab() {
  ...
  return (
    <div>
      {error && (
        <div className="mx-8 mt-4 rounded border border-invalid-edge bg-invalid-soft px-4 py-2 text-sm text-invalid">
          {error}
        </div>
      )}
            ...
            <div className="divide-y divide-zinc-800">
              {recent.map((r) => (
                <div key={r.id} className="py-3 first:pt-0 last:pb-0">
                  ...
                  <div className="mt-1 text-sm text-text">{r.content}</div>
                  {r.why && (
      ...
      <footer className="px-8 pb-8 text-xs text-text-dim">
        Sources: Postgres (threads) and LanceDB (memory records).
      </footer>
    </div>
  );
}
...
function KpiCard({
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-5">
      ...
      <div className="mt-2 text-3xl font-semibold tabular-nums text-text">
        {value}
      </div>
      {hint && (
        <div className="mt-1 line-clamp-1 text-xs text-text-subtle">{hint}</div>
      )}
    </div>
  );
}
